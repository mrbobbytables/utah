...
1. **Cheap deterministic validation for ordinary PRs:** Pull requests run only
   the `contract` job (`just check`, `just check-parity`, `just check-repos`, and
   flavor resolution). PRs never run the expensive image builds (`build_main`,
   `build_kernel`, `kernel_cache`).
2. **Merge queue performs full release-candidate builds:** When a PR enters the
   ...
   matrix: `contract`, `kernel_cache`, `build_main`, and `build_kernel`.
...
4. **Explicit timeouts and log preservation:** Every job defines an explicit
   `timeout-minutes`. Long-running tasks capture build output and upload logs as
   artifacts on completion or failure (e.g. `kernel-cache-build-logs`).
